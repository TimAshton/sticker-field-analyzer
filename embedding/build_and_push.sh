#!/bin/bash
# Builds and pushes the shared CLIP-embedding image (embed_known.py +
# match.py) used by both the embed-known and match Lambdas.
#
# Run this from embedding/, then re-apply Terraform with the printed tag:
#   cd embedding && ./build_and_push.sh
#   cd ../terraform/environments/dev && terraform apply -var="embedding_image_tag=<tag printed above>"
#
...
REGION=us-west-2
REPO="sticker-field-analyzer-embedding"
...
echo "Pushed $IMAGE"
echo "Apply with: terraform apply -var=\"embedding_image_tag=${TAG}\""
