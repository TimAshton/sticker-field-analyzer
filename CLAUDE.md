...
Detection/cropping — splitting a field photo into individual stickers, ported from `app/`'s prototype — is wired up via the `detect` Lambda (`detection/detect.py`, see the constraints section below for how it hooks into the same upload event as `ingest`). It writes `CROP#` items and crop images to S3 under `crops/<image_id>/<crop_id>.jpg`, but deliberately doesn't touch the image's `METADATA` status (avoiding a write race with `ingest`). Those crop writes are also what feeds `match` (below) — matching per-crop instead of per-whole-photo was the entire motivation for building this stage.
...
- `ingest` and `detect` both react to the same raw `stickers/` upload event independently and in parallel — they don't call each other or depend on each other's output. `match` is downstream of `detect` instead: it reacts to `detect`'s `crops/` writes (via its own EventBridge rule, same delivery mechanism as `detect`'s — see `detect_lambda.tf`/`match_lambda.tf`), not the original upload event directly.
